...
content and real Pair Again navigation are implemented. The package suite passes 1,436 tests in
135 suites, unsigned package/app builds and the real pinned-HTTPS regression suite pass. New snapshots cover loading routes, long waits,
...
changed PNG has been reviewed. Accessibility loading snapshots capture the first native activity
animation frame while preserving actual text sizes, normal window rendering and the existing image
...
All six local coverage values hold against main at `73a5ebe`: unit lines/regions 97.2%/94.5%,
snapshot lines/regions 97.1%/97.7%, and all-test lines/regions 98.0%/95.3%.
GitHub CLI authentication works without machine configuration changes. [PR #84](https://github.com/fardavide/granita/pull/84)
is open; its first CI run exposed legacy reporting contracts that lacked coverage and native
accessibility spinner phase drift. Those fixes are verified locally and awaiting a new CI run.
Auto-merge is disabled so the fixes can complete CI and the PR remains open for review.
No release has been published.
...
connected. The phone keeps that endpoint beside the pairing, continues to prefer Bonjour on the LAN,
and falls back directly when discovery is empty, unreachable or denied. Existing pairings learn the
endpoint from pinned health on their next successful local connection; no re-pairing or Tailscale
