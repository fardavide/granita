...
- **Both halves are designed.** `.ai/docs/design.md` is the authority on what the phone and the
  iPad look like, `.ai/docs/design-mac.md` on the menu bar app's seven surfaces, and the
...
`.ai/rules/ast-index.md` has the command table. Fall back to `rg`/`grep` only for regex, string
