...
actor FakePinnedServerHealthChecking: PinnedServerHealthChecking {
    ...
    init(answering: HealthResponse, answers: [ServerAddress: Result<HealthResponse, ApiFailure>] = [:], waitingFor: [ServerAddress: ServerAddress] = [:], beforeAnswering: @escaping @Sendable () async -> Void = {}) {
    ...
    func health(at address: ServerAddress, pinnedTo fingerprint: SpkiFingerprint) async throws(ApiFailure) -> HealthResponse {
        ...
        switch answers[address] ?? .success(answering) {
